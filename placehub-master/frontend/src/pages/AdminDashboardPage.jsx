    ...
    return (
        <MainLayout>
            <div className="admin-dashboard-root">
                ...
                {/* Real-time Indicator */}
                <div className="fixed top-24 right-8 z-50 flex items-center gap-2 px-3 py-1 bg-white/80 backdrop-blur shadow-sm border border-gray-100 rounded-full">
                ...
                <div className="admin-view-header">
                    <div className="admin-title-area">
                        ...
                        <p className="text-sm font-bold opacity-60 flex items-center gap-2">
                            ...
                            {view.charAt(0).toUpperCase() + view.slice(1)} Logistics
                        </p>
                    </div>
                    <div className="flex gap-3">
                        <button className={`pro-btn rounded-xl px-6 ${view === 'overview' ? 'bg-primary text-white shadow-lg' : ''}`} onClick={() => { setView('overview'); setSubView(null) }}>Dashboard</button>
                        <button className={`pro-btn rounded-xl px-6 ${view === 'meetings' ? 'bg-primary text-white shadow-lg' : ''}`} onClick={() => { setView('meetings'); setSubView(null) }}>Sessions</button>
                        <button className={`pro-btn rounded-xl px-6 ${view === 'logs' ? 'bg-primary text-white shadow-lg' : ''}`} onClick={() => { setView('logs'); setSubView(null) }}>System Logs</button>
                    </div>
                </div>
                ...
                {subView && (
                    <div className="space-y-6">
                        <button className="pro-btn flex items-center gap-2 mb-4 hover:bg-slate-50" onClick={() => setSubView(null)}><X size={18} /> Back to Dashboard</button>
                            ...
                            <div className="bg-white rounded-[2rem] shadow-xl border border-gray-100 overflow-hidden animate-in zoom-in-95 duration-300">
                                ...
                                <div className="p-10">
                                    {subView === 'student_detail' ? (
                                            ...
                                            <div className="space-y-8">
                                                <div>
                                                    ...
                                                    <div className="space-y-3 font-bold text-slate-700">
                                                        <p className="flex items-center gap-3"><Mail size={16} className="text-secondary" /> {selectedItem?.profile?.collegeEmail}</p>
                                                        <p className="flex items-center gap-3"><Phone size={16} className="text-secondary" /> {selectedItem?.profile?.whatsappNumber || 'N/A'}</p>
                                                        <p className="flex items-center gap-3"><Linkedin size={16} className="text-secondary" /> <a href={selectedItem?.profile?.linkedinUrl} target="_blank" className="text-secondary hover:underline">LinkedIn Profile</a></p>
                                                    </div>
                                                </div>
                                                <div>
                                                    <h3 className="text-sm font-black text-slate-400 uppercase tracking-widest mb-4">Skills & Expertise</h3>
                                                    <div className="flex flex-wrap gap-2">
                                                        {selectedItem?.profile?.skills?.map((s, i) => (
                                                            <span key={i} className="px-4 py-1.5 bg-slate-50 text-slate-600 rounded-full border border-slate-100 text-sm font-bold">{s}</span>
                                                        ))}
                                                    </div>
                                                </div>
                                            </div>
                                            <div className="space-y-8">
                                                <div>
                                                    ...
                                                    <div className="p-6 bg-slate-50 rounded-2xl border border-slate-100">
                                                        {selectedItem?.profile?.placementStatus === 'placed' ? (
                                                            <div className="space-y-4">
                                                                <div>
                                                                    <p className="text-xs font-bold text-slate-400 uppercase">Company</p>
                                                                    <p className="text-xl font-black text-primary">{selectedItem?.profile?.company}</p>
                                                                </div>
                                                                <div>
                                                                    <p className="text-xs font-bold text-slate-400 uppercase">Role</p>
                                                                    <p className="text-lg font-bold text-secondary">{selectedItem?.profile?.role}</p>
                                                                </div>
                                                            </div>
                                                        ) : (
                                                            <p className="font-bold text-slate-500">Currently preparing for placements.</p>
                                                        )}
                                                    </div>
                                                </div>
                                                <div className="grid grid-cols-2 gap-4">
                                                    <div className="p-6 bg-blue-50/50 rounded-2xl border border-blue-100 text-center">
                                                        <p className="text-2xl font-black text-blue-600">{selectedItem?.meetings?.length || 0}</p>
                                                        <p className="text-[10px] font-black text-blue-400 uppercase tracking-widest mt-1">Sessions</p>
                                                    </div>
                                                    <div className="p-6 bg-purple-50/50 rounded-2xl border border-purple-100 text-center">
                                                        <p className="text-2xl font-black text-purple-600">{selectedItem?.problems?.length || 0}</p>
                                                        <p className="text-[10px] font-black text-purple-400 uppercase tracking-widest mt-1">Contributions</p>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div className="space-y-8">
                                            <div>
                                                <h3 className="text-sm font-black text-slate-400 uppercase tracking-widest mb-4">Problem Content</h3>
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                                <div className="p-6 bg-slate-50 rounded-2xl border border-slate-100">
                                                    <p className="text-xs font-bold text-slate-400 uppercase mb-2">Uploader</p>
                                                    <p className="font-black text-primary">{selectedItem?.profile?.fullName || 'Anonymous'}</p>
                                                    <p className="text-sm font-bold text-slate-500">{selectedItem?.profile?.collegeEmail}</p>
                                                </div>
                                                <div className="p-6 bg-slate-50 rounded-2xl border border-slate-100">
                                                    <p className="text-xs font-bold text-slate-400 uppercase mb-2">Target Company</p>
                                                    <p className="font-black text-primary">{selectedItem?.problem?.company || 'Generic'}</p>
                                                </div>
                                                <div className="p-6 bg-slate-50 rounded-2xl border border-slate-100">
                                                    <p className="text-xs font-bold text-slate-400 uppercase mb-2">Category</p>
                                                    <div className="flex flex-wrap gap-1 mt-1">
                                                        {selectedItem?.problem?.tags?.map((t, i) => (
                                                            <span key={i} className="px-2 py-0.5 bg-white text-[10px] font-bold rounded border border-slate-200">{t}</span>
                                                        ))}
                                                    </div>
                                                </div>
                                            </div>
